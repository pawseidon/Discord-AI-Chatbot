...
class ContextManager:
    ...
    def find_related_thinking(self, query: str, limit: int = 3) -> List[Tuple[str, str, Optional[str], Dict[str, Any]]]:
        ...
        for (user_id, channel_id, thread_id), _ in sorted_matches:
            ...
            if key in self.conversations:
                ...
                if thinking_idx < len(conversation.thinking_history):
    ...
    async def save_to_disk(self) -> None:
        ...
        try:
            ...
            # Save to file
            state_file = os.path.join(self.storage_dir, "context_state.json")
    ...
    async def load_from_disk(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(state_file):
                logger.info(f"No context state file found at {state_file}")
                return False
            ...
            for key, conv_data in state.get("conversations", {}).items():
                ...
                conversation.last_active = conv_data["last_active"]
                conversation.context_map = conv_data["context_map"]
                conversation.thinking_history = conv_data["thinking_history"]
                conversation.related_threads = conv_data["related_threads"]
                conversation.summary = conv_data["summary"]
                conversation.topic = conv_data["topic"]
                
                self.conversations[key] = conversation
                
            # Rebuild thinking index
            self.thinking_index = defaultdict(list)
            for key, conversation in self.conversations.items():
                user_id = conversation.user_id
                channel_id = conversation.channel_id
                thread_id = conversation.thread_id
                ...
                for i, thinking in enumerate(conversation.thinking_history):
                    if "problem" in thinking:
                        ...
                        for keyword in keywords:
                            if len(keyword) > 3:  # Only index meaningful keywords
                                self.thinking_index[keyword].append((user_id, channel_id, thread_id, i))
    ...
    async def scheduled_save(self, interval: int = 300) -> None:
        """Run a scheduled task to periodically save context to disk"""
        while True:
            await asyncio.sleep(interval)
            await self.save_to_disk()
            
    async def cleanup_old_conversations(self, max_age: int = 86400) -> None:
        """Remove conversations older than the specified age (in seconds)"""
        ...
        for key, conversation in self.conversations.items():
            if current_time - conversation.last_active > max_age:
                ...
                # Also remove from thread map if needed
...
async def start_context_manager_tasks():
    """Start background tasks for the context manager"""
    context_manager = get_context_manager()
    
    # Load existing state from disk
    await context_manager.load_from_disk()
    
    # Start scheduled tasks
    asyncio.create_task(context_manager.scheduled_save())
    asyncio.create_task(context_manager.cleanup_old_conversations()) 